...
if (titleHeader instanceof HTMLElement &&
    ...
    inputInteractDOM.setAttribute("placeholder", lang === "es" ? 'selecciona "portafolio"' : 'select "portfolio"');
    ...
    if (formHeader instanceof HTMLElement &&
        ...
        nameInput.setAttribute("placeholder", lang === "es" ? "tu nombre" : "your name");
        emailLabel.textContent = lang === "es" ? "Correo electrónico" : "Email";
        emailInput.setAttribute("placeholder", lang === "es" ? "tu correo electrónico" : "your email");
        messageLabel.textContent = lang === "es" ? "Tu mensaje" : "Your message";
        messageTextarea.setAttribute("placeholder", lang === "es" ? "tu mensaje" : "your message");
        submitButton.textContent = lang === "es" ? "enviar" : "send";
    }
    if (englishSkillsHeader instanceof HTMLElement &&
...
const inputInteract = document.querySelector("#input-interact-DOM");
let i = 0;
const suggestionsArr = lang === "es"
    ? [
...
if (inputInteract) {
    setTimeout(() => {
        let i = 0;
        let selectPortfolio = lang === "es" ? 'selecciona "portafolio"' : 'select "portfolio"';
...
inputInteract?.addEventListener("keyup", (event) => {
    ...
    if (inputInteract.value.length === 0 &&
...
async function createElementFromInput() {
    ...
    if (typeof inputInteract?.value === "string") {
        handleInputErrors(inputInteract.value, selectElementArr, deleteWordsArr, createWordsArr, textElementsArr, containersArr);
    }
    let create = false;
    let select = false;
    ...
    if (inputInteract) {
        ...
        if (select &&
            ...
            !select &&
            !create) {
            let elementSelected = document.querySelector(querySelectordeleteClassName);
...
function handleInputErrors(inputText, selectElementArr, deleteWordsArr, createWordsArr, textElementsArr, containersArr) {
    if (inputInteract) {
        ...
        for (let i = 0; i < inputWordArr.length; i++) {
            ...
            if ((textElementsArr.includes(word) || containersArr.includes(word)) &&
            ...
            else if ((textElementsArr.includes(word) || containersArr.includes(word)) &&
...
function showErrorAndCorrect(inputInteract, errorMessage, inputWordArr, index) {
    let iteration = 0;
    let interval = setInterval(() => {
        iteration++;
...
async function parseHtmlElements(element, depth, parentElement) {
    return new Promise(async (resolve) => {
        ...
        if (element.firstElementChild) {
            ...
            if (element.firstChild &&
                ...
                spanTextNode.textContent = `${element.firstChild.textContent.trim() ?? ""}`;
            ...
            for (let i = 0; i < element.children.length; i++) {
                ...
                if (child instanceof HTMLElement && child.firstElementChild) {
                    ...
                    spanChild.appendChild(document.createTextNode(`>${childTextContent ?? ""}</`));
            ...
            span.appendChild(document.createTextNode(`>${elementTextContent ?? ""}</`));
            const spanTagEnd = document.createElement("span");
            spanTagEnd.className = "tagEnd red";
            spanTagEnd.textContent = elementTag;
        ...
        if (divElements.firstElementChild instanceof HTMLElement) {
            ...
            setTimeout(async () => {
                const isSection = divElements.firstElementChild instanceof HTMLElement &&
        ...
        for (let i = 0; i < elementContainer.childNodes.length; i++) {
            ...
            if (child instanceof HTMLElement &&
                ...
                for (let j = 0; j < child.childNodes.length; j++) {
                    promises.push(randomLetterAnimation(child.childNodes[j], generateText));
                }
            }
...
async function convertToHtml(element, HTMLInstance) {
    return new Promise(async (resolve) => {
        const divElements = document.createElement("div");
        ...
        if (element.classList.contains("section-3")) {
            ...
            const inputName = formSendEmail?.querySelector("#name");
            const inputEmail = formSendEmail?.querySelector("#email");
            const textareaMessage = formSendEmail?.querySelector("#message");
            function updateButtonState() {
                let allFilled = inputName &&
                    inputEmail &&
                    textareaMessage &&
